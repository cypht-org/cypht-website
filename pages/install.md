...
BASE_DIR="/usr/local/share/cypht"
DEFAULT_VERSION=""  # Placeholder for default version, will be set dynamically

# Function to check prerequisites
check_prerequisites() {
    if ! command -v composer &>/dev/null; then
        echo "Error: Composer is not installed or not in the system PATH."
        echo "Please install Composer before proceeding: https://getcomposer.org/download/"
        exit 1
    fi
}
...
fetch_tags() {
    curl -s https://api.github.com/repos/cypht-org/cypht/tags | \
    jq -r '.[].name' | sort -V
}
...
install_cypht() {
    ...
    # Check if /usr/local/share/ exists, if not, ask the user to change BASE_DIR
    if [ ! -d "/usr/local/share" ]; then
        echo "Error: /usr/local/share/ does not exist. Please change the BASE_DIR in the script to a valid path."
        exit 1
    fi
    ...
    echo "Creating directory for version $version: $destination"
    ...
    # Ensure the version string starts with a single "v" (e.g., v1.4.0, not vv1.4.0)
    version_cleaned=$(echo "$version" | sed 's/^v*//')

    # Download the selected version of Cypht
    wget "https://github.com/cypht-org/cypht/archive/refs/tags/v$version_cleaned.zip" -O "v$version_cleaned.zip"
    if [ $? -ne 0 ]; then
        echo "Error downloading version v$version_cleaned."
        exit 1
    fi

    # Unpack the archive
    unzip "v$version_cleaned.zip"
    if [ $? -ne 0 ]; then
        echo "Error unpacking v$version_cleaned.zip."
        exit 1
    fi
    extracted_folder="cypht-$version_cleaned"
    ...
    if [[ "$version_cleaned" =~ ^1\.[0-9]+\.[0-9]+$ ]]; then
        echo "Detected version $version_cleaned. Creating .env from .env.example."
        cp .env.example .env
    else
        echo "Detected version $version_cleaned. Creating .env from .env.example."
        cp .env.example .env
    ...
    read -p "Enter the group to own the files (default is 'root'): " group
    group="${group:-root}"
    ...
    echo "Cypht $version_cleaned installed successfully to $destination"
}

# Check prerequisites
check_prerequisites

# Fetch available version tags and get the latest one as the default
available_versions=$(fetch_tags)
DEFAULT_VERSION=$(echo "$available_versions" | tail -n 1)

echo "Available Cypht versions:"
echo "$available_versions" | nl -s '. '

# Menu for version selection
echo "Select the version of Cypht you want to install:"
echo "1. Install Cypht (specific version)"
echo "2. Use the default version ($DEFAULT_VERSION)"
read -p "Enter your choice (1 or 2): " choice

if [ "$choice" -eq 1 ]; then
    # List the available versions for selection
    read -p "Enter the number of the version from the list above (e.g., 1 for v1.0.0): " version_choice

    # Get the version based on the user’s choice
    selected_version=$(echo "$available_versions" | sed -n "${version_choice}p")

    if [ -z "$selected_version" ]; then
        echo "Error: Invalid version choice. Please select a valid number from the list."
        exit 1
    fi
