...
const load_doc_menu = (nav_id) => {
  ...
  const menu_items = menus
    .map((item) => {
      const item_id = item.title.toLowerCase().replace(/ /g, "-");
      const item_collapse_id = `${item_id}-collapse`;
      return `<div class="toc-section accordion-item">
                <h5 class="accordion-header">
                  <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#${item_collapse_id}" aria-expanded="true" aria-controls="${item_collapse_id}">
                    ${item.title}
                  </button>
                </h5>
                <div id="${item_collapse_id}"
                  class="accordion-collapse collapse" data-bs-parent="#doc_menu">
