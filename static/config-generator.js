...
function addExpandLabel(jqueryElem, prefix = "Entering a value") {
  ...
  jqueryElem.after(
    `<small>${prefix} in the above input will change other presented options!</small>`
  );
}
...
function resetInput(jqueryElement, checkbox = false) {
  if (!checkbox) {
    ...
    jqueryElement
      .prop("checked", jqueryElement.data("default"))
      .change()
      .trigger("input");
  }
}
...
function onClickGenerate(value) {
  ...
  allInputs.each(function (i) {
    const input = $(this);
    const value =
      input.attr("type") != "checkbox" ? input.val() : input.is(":checked");
  ...
  Array.from(textLines).forEach((line, i) => {
    ...
    if (
      ...
      (line.startsWith("DEFAULT_SMTP") &&
        lastLine.startsWith("DEFAULT_SETTING"))
    ) {
      textLines[i - 1] += "\n";
    }
    lastPrefix = prefix;
